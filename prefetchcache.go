...
// PrefetchCache manages a key:value store, refreshing (fetching fresh) items using the ItemFetcher
...
func NewCache(fetcher ItemFetcher, globalTimeout *time.Duration, timeout ItemTimeout) (Cache, error) {
	...
	c := &PrefetchCache{store: NewConcurrentMapStore()}
...
func (c *PrefetchCache) WithStore(newStore func() ConcurrentKVStore) Cache {
	c.store = newStore()
	return c
}
...
func (c *PrefetchCache) WithGlobalRefreshInterval(refresh *time.Duration) Cache {
...
func (c *PrefetchCache) WithGlobalTTL(timeToLive *time.Duration) Cache {
...
func (c *PrefetchCache) WithChannelBasedMonitor(
	...
	c.monitor = NewChannelBasedMonitor(
		newMonitor(), hitChSize, missChSize, refreshChSize, setChSize, evictChSize,
	)
	return c
}

// WithMonitor builds a cache with the provided monitor
func (c *PrefetchCache) WithMonitor(newMonitor func() Monitor) Cache {
...
func (c *PrefetchCache) Get(key interface{}) (interface{}, error) {
	c.lock.RLock()
...
func (c *PrefetchCache) get(key interface{}) (bool, interface{}, error) {
...
func (c *PrefetchCache) refreshKey(key interface{}, nonce string) (interface{}, error) {
	c.lock.RLock()
	defer c.lock.RUnlock()
	if c.closed {
		return nil, errClosed
...
func (c *PrefetchCache) set(key interface{}, refreshInterval, timeToLive *time.Duration, value interface{}, nonce string, fetchErr error) {
	s := time.Now()
...
func (c *PrefetchCache) resolveControls(refreshInterval, timeToLive *time.Duration) (*time.Duration, *time.Duration) {
	r := c.refreshInterval
	ttl := c.timeToLive
